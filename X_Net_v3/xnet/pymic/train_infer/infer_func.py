...
def volume_infer(image_ct, image_mr, net, device, class_num, 
    ...
    """
    image_ct = image_ct.to(device)
    image_mr = image_mr.to(device)
    if(mini_patch_inshape is None):
        outputs_ct, outputs_mr = net(image_ct, image_mr)
        outputs_ct = outputs_ct.cpu().numpy()
        outputs_mr = outputs_mr.cpu().numpy()
        #outputs, attmap = net(image)
        #outputs = outputs.cpu().numpy()
        #attmap = attmap.cpu().numpy()
    else:
        outputs_ct, outputs_mr = volume_infer_by_patch(image_ct, image_mr, net, device, class_num,
            mini_batch_size, mini_patch_inshape, mini_patch_outshape)
    #return outputs, attmap
    return outputs_ct, outputs_mr

def volume_infer_by_patch(image_ct, image_mr, net, device, class_num,
        mini_batch_size, mini_patch_inshape, mini_patch_outshape):
    '''
        ...
        '''
    image_ct = image_ct.cpu().numpy()
    image_mr = image_mr.cpu().numpy()
    img_full_shape = image_ct.shape # image_ct and image_mr have the same size (at least they should!) 
    ...
    if(max(margin) > 0):
        ...
        image_pad_ct = np.pad(image_ct, pad, 'reflect')
        image_pad_mr = np.pad(image_mr, pad, 'reflect')
    else:
        margin_lower = [0] * img_dim
        margin_upper = [0] * img_dim
        image_pad_ct = image_ct
        image_pad_mr = image_mr
    [padD, padH, padW] = image_pad_ct.shape[2:]
    nd = math.ceil(D/mini_patch_outshape[0])
    nh = math.ceil(H/mini_patch_outshape[1])
    nw = math.ceil(W/mini_patch_outshape[2])
    sub_image_patches_ct = []
    sub_image_patches_mr = []
    sub_image_starts  = []
    ...
    for di in range(nd):
        ...
        for hi in range(nh):
            ...
            for wi in range(nw):
                ...
                sub_image_ct = crop_ND_volume_with_bounding_box(image_ct, 
                    crop_start_full, crop_end_full)
                sub_image_mr = crop_ND_volume_with_bounding_box(image_mr, 
                    crop_start_full, crop_end_full)
                sub_image_patches_ct.append(sub_image_ct)
                sub_image_patches_mr.append(sub_image_mr)

    # inference with image patches
    out_shape = [img_full_shape[0], class_num] + list(image_pad_ct.shape[2:])
    out = np.zeros(out_shape, np.float32)
    total_batch = len(sub_image_patches_ct)
    ...
    for mini_batch_idx in range(max_mini_batch):
        ...
        data_mini_batch_ct = sub_image_patches_ct[batch_start_idx:batch_end_idx]
        data_mini_batch_mr = sub_image_patches_mr[batch_start_idx:batch_end_idx]
        data_mini_batch_ct = np.concatenate(data_mini_batch_ct, axis = 0)
        data_mini_batch_mr = np.concatenate(data_mini_batch_mr, axis = 0)
        data_mini_batch_ct = torch.from_numpy(data_mini_batch_ct)
        data_mini_batch_mr = torch.from_numpy(data_mini_batch_mr)
        data_mini_batch_ct = torch.tensor(data_mini_batch_ct)
        data_mini_batch_mr = torch.tensor(data_mini_batch_mr)
        data_mini_batch_ct = data_mini_batch_ct.to(device)
        data_mini_batch_mr = data_mini_batch_mr.to(device)
        out_mini_batch_ct, out_mini_batch_mr  = net(data_mini_batch_ct, data_mini_batch_mr).cpu().numpy()
        ...
        for batch_idx in range(batch_start_idx, batch_end_idx):
            ...
            out_ct = set_ND_volume_roi_with_bounding_box_range(out, crop_start, crop_end, 
                     out_mini_batch_ct[batch_idx-batch_start_idx])
            out_mr = set_ND_volume_roi_with_bounding_box_range(out, crop_start, crop_end, 
                     out_mini_batch_mr[batch_idx-batch_start_idx])

    if(max(margin) > 0):
        crop_start = [0, 0] + margin_lower
        crop_end   = [img_shape[i] - margin_upper[i] for i in range(img_dim)]
        crop_end   = [1, class_num] + crop_end
        out_ct = crop_ND_volume_with_bounding_box(out_ct, crop_start, crop_end) 
        out_mr = crop_ND_volume_with_bounding_box(out_mr, crop_start, crop_end) 
    return out_ct, out_mr
